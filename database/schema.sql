...
CREATE TABLE `instalacions` (
    ...
    `moduls` JSON DEFAULT NULL COMMENT 'NULL = tots; ["espais","torns","equips"]',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uniq_instalacions_nom` (`nom`)
...
-- Enllaços d'accés d'un sol ús (activació i canvi de contrasenya). Només es guarda el SHA-256.
DROP TABLE IF EXISTS `usuari_tokens`;
CREATE TABLE `usuari_tokens` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `usuari_id` INT UNSIGNED NOT NULL,
...
-- -----------------------------------------------------------
-- ASSIGNACIÓ COMPARTIDA DE TASQUES DEL PLA A DIVERSOS TORNS
-- -----------------------------------------------------------
DROP TABLE IF EXISTS `tasca_pla_torn`;
CREATE TABLE `tasca_pla_torn` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
