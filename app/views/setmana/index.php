...
$queryBase = 'setmana=' . $setmanaOffset . ($tornActual ? '&torn=' . $tornActual : '') . ($search ? '&q=' . urlencode($search) : '');
...
<div class="mb-4 max-w-full overflow-x-hidden">
    <form method="GET" action="<?= url('setmana') ?>" class="flex flex-col sm:flex-row gap-2">
        ...
        <?php if ($search): ?>
            <a href="<?= url('setmana?setmana=' . $setmanaOffset . ($tornActual ? '&torn=' . $tornActual : '')) ?>" class="bg-gray-100 text-gray-500 px-4 py-2 rounded-lg text-sm hover:bg-gray-200 transition text-center">Netejar</a>
        <?php endif; ?>
    </form>
</div>
...
<div class="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-3 mb-3 max-w-full overflow-x-hidden">
    <div class="flex items-stretch gap-2 w-full sm:w-auto min-w-0 max-w-full">
        ...
        <div class="min-w-0 flex-1 bg-white border border-gray-200 rounded-lg px-3 py-2 text-center">
            ...
            <div class="text-[11px] sm:text-xs text-gray-500 leading-tight">
        ...
        <a href="<?= url('setmana' . ($tornActual ? '?torn=' . $tornActual : '') . ($search ? ($tornActual ? '&' : '?') . 'q=' . urlencode($search) : '')) ?>"
           class="hidden sm:inline text-sm text-brand hover:text-brand-dark transition ml-1">Avui</a>
        <?php endif; ?>
    </div>
    <?php if ($setmanaOffset !== 0): ?>
    <a href="<?= url('setmana' . ($tornActual ? '?torn=' . $tornActual : '') . ($search ? ($tornActual ? '&' : '?') . 'q=' . urlencode($search) : '')) ?>"
    ...
    <div class="flex flex-wrap items-center gap-2 w-full min-w-0 max-w-full">
        ...
        <a href="<?= url('setmana?setmana=' . $setmanaOffset . '&torn=' . $t['id'] . ($search ? '&q=' . urlencode($search) : '')) ?>"
...
<div class="flex flex-wrap items-center gap-2 mb-3 text-sm">
    ...
    <?php endif; ?>
    <?php if ($nSetmana > 0): ?>
        <span class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs font-medium"><?= $nSetmana ?> de la setmana</span>
    <?php endif; ?>
</div>
<?php endif; ?>
