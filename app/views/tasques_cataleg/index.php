...
<div class="flex items-center justify-between mb-6">
    ...
    <a href="<?= url('tasques-cataleg/create') ?>" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition flex items-center gap-2">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <?php else: ?>
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-4 py-3 font-mono text-xs text-blue-600"><?= e($tasca['codi'] ?? '-') ?></td>
                        <td class="px-4 py-3">
                            <div class="max-w-md truncate" title="<?= e($tasca['nom']) ?>"><?= e($tasca['nom']) ?></div>
                        </td>
                        <td class="px-4 py-3">
                            <?php if (!empty($tasca['sistema_codi'])): ?>
                                <span class="inline-block bg-blue-50 text-blue-700 text-xs px-2 py-0.5 rounded"><?= e($tasca['sistema_codi']) ?></span>
                            <?php else: ?>
                        ...
                        <td class="px-4 py-3 text-right">
                            ...
                            <div class="flex items-center justify-end gap-2">
                                <a href="<?= url('tasques-cataleg/edit/' . $tasca['id']) ?>" class="text-gray-400 hover:text-blue-600 transition" title="Editar">
